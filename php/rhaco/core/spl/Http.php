...
class Http extends Object{
	...
	private function build_url($url){
		...
		if($this->_base_url_ !== null) return File::absolute($this->_base_url_,(substr($url,0,1) == "/") ? substr($url,1) : $url);
		return $url;
	}
	/**
	 * getでアクセスする
	...
	private function request($url,$method,array $header=array(),array $vars=array(),$download_path=null,$status_redirect=true){
		...
		if($fp == false || false == stream_set_blocking($fp,1) || false == stream_set_timeout($fp,$this->timeout)) throw new InvalidArgumentException("connection fail [".$url."] ".$errormsg." ".$errorno);
